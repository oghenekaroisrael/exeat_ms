...
error_reporting(0);
	...
	public function loan_application($amount,$purpose, $rate, $paccount, $sone, $stwo, $user){
		try {
			$stmt = $this->db->prepare("INSERT INTO loan_applications(amount,purpose,rate_id,account_id,surety_one,surety_two,applicant_id,date_created) 
			VALUES (?,?,?,?,?,?,?,NOW())");
			$stmt->execute([$amount,$purpose, $rate, $paccount, $sone, $stwo, $user]);
