<?php
	...
	function newUser(){
		...
		$matricNumber = lcfirst(htmlspecialchars($_POST['matricNumber']));
		$matricNumber = lcfirst(stripslashes($_POST['matricNumber']));
		$matricNumber = lcfirst(trim($_POST['matricNumber']));
		...
		$level = lcfirst(htmlspecialchars($_POST['level']));
		$level = lcfirst(stripslashes($_POST['level']));
		$level = lcfirst(trim($_POST['level']));
		...
		if($validator->ValidateForm()){
			...
			if($error){
				...
				$insert = Database::getInstance()->insert_user($first,$middle, $last, $email, $role, $hash,$chapel,$hall, $department,$matricNumber,$level,$pnumber);
				echo $insert;
			}
		} else {
	...
	function login(){
		...
		$username = lcfirst(htmlspecialchars($_POST['email']));
		$username = lcfirst(stripslashes($_POST['email']));
		$username = lcfirst(trim($_POST['email']));
		...
		//if($validator->ValidateForm()){
			...
			if (!empty($username) && !empty($password)) {
				...
				echo Database::getInstance()->check_pass($username, $password);
			}
		// } else {
		// 	$error_hash = $validator->GetErrors();
		// 	foreach($error_hash as $inpname => $inp_err){
	...
	function getWithdrawable(){
		...
		$user = $_POST['user'];
			...
			if($prev_withdrawal != NULL && $sixMonths > date('Y-m-d')){
				...
				$res = Database::getInstance()->get_withdrawal_limit($user,$amount);
			}else if($prev_withdrawal == NULL && $sixMonths <= date('Y-m-d')){
				$res = Database::getInstance()->get_withdrawal_limit($user,$amount);
			}else if($prev_withdrawal == NULL && $sixMonths > date('Y-m-d')){
				$res = 'No';
	...
	function addRemark(){
		...
		$remark = $_POST['remark'];
			...
			if (empty($saving_id)) {
				...
				echo Database::getInstance()->addRemark($saving_id,$remark,$user);
			}			
	}
